...
## Revision on 2026-09-18: the agent tile becomes a 2x2

Malik asked for "a 2x2 with the grok bot in the 4th cell", and for Claude and Codex to be shown as apps rather than terminals.

- **Grok Bot is not in it.** Its bots each run on their own cloud desktop (the Motion Bot's screen is a remote Linux desktop with Chrome and a terminal), so a bot cannot open inspire-ocean or the capture image on the Mac, and a take would show it failing, or going to GitHub instead. The fourth cell is Antigravity, which runs on the Mac and works in a local folder (Malik: "try antigravity").
- **Codex is its app** (the ChatGPT app's Work mode, GPT-6 Astra). **Claude Code stays the terminal for now** (Malik: "leave claude as terminal for now"): the Claude app is the one running the working session, so it could not be driven for a take.
- **How the new takes were made.** One window each, recorded with ScreenCaptureKit on inspire-ocean on Sep 18. The exact payload Locant's MCP server returns for capture `20260915-021609-zwec` was pasted with cmd+V, sent, and the agent stopped as soon as it had started: Codex after "I'll inspect the captured orb and the existing Light And Color design..." and "Viewed an image"; Antigravity after it analyzed the capture, searched Product Ideas and Light and Color, and opened `SeedScreenshot.swift`. inspire-ocean was checked against a snapshot (HEAD, status, diff) after each take and never changed. Antigravity asked once for leave to read the capture image, which is outside its project; that one read was allowed, and the tile skips the wait.
- **The tile.** `locant-tile-agent.mp4` is four cells of 596x446 on one 6.0 s clock: 0.6 s of empty input, the paste at 0.6, the send at 1.3, then 4.7 s of each agent starting. Everything plays at 1x; waits are cut only where the frame is still, and a cell changes its crop only at the send, where the app's own layout jumps. Cursor's cell opens with the payload already pasted, because its paste lands while the film's camera is still arriving from the Simulator. The file opens at 5.05 s with all four at work, which is also the poster and the reduced-motion still, and its 0.4 s dissolve joins that back to the empty inputs. 536 KB. Built by `agents_tile2.py` (session scratchpad, not in the repo).

## Revision on 2026-09-19: apps in Final Design, and the idle pointer

Malik asked for the apps in Final Design too, and whether any rule governed the cursor hovering in the recordings.

- **Final Design's "Any agent" clip is the Codex app, then Antigravity** (`locant-agents.mp4`, 16.7 s, 1920x1080), cut from the same Sep 18 takes as the tile under the same rules: 1x throughout, waits cut only where the frame is still, a crop change only at the send. Codex pastes, sends, and gets to "Viewed an image"; Antigravity opens on its composer already holding the payload (its paste happened under two onboarding notices), sends, and runs down its search list to `SeedScreenshot.swift` and `OceanScene.swift`. The one wait for leave to read the capture image is cut. Sidebars stay out of frame, Codex's because it lists other chats. It replaces the Codex CLI and Claude Code terminal cutaways (E1, E2).
- **The rule that existed.** The film's contract (`06-film-script.md`) bakes the real pointer into every take and adds click rings in the composite, and its cursor rules govern motion only: curved, minimum-jerk paths, a soft settle, a beat of stillness before each click. Nothing said where the pointer should rest, so after clicking into an input it stayed parked there.
- **The cleanup.** Two scenes carried a parked pointer into the agent clips. In scene A, Cursor's I-beam sits on "137×182" from 27.2 s to the send, with a click ring at 27.6 s; in scene E2 the I-beam rests mid-terminal from 0.3 s to 4.4 s, with a ring at 1.5 s. Both sit over pixels that do not change until the send, and each take has a clean frame of them: A at 29.00 s (the pointer hides on the Enter keydown, one frame before the chat box collapses), E2 at 0.00 s (before the pointer arrives). That frame's rectangle covers the pointer, laid in YUV at even offsets so nothing is colour-converted; outside the pointer the rectangle matched every frame it covers to within 0.35 grey levels. Nothing is drawn or reconstructed. The tile and the Highlights clip use the cleaned scenes; the Highlights clip now starts at 27.22 s, once the camera has settled, so the patch is valid for every frame it covers. Every other clip was checked: the pointer is the interaction in the hero, the hovers, Before & After, the ring and Color, and typing hides it in the Before clip. The new app takes were recorded without a pointer, and it never sat inside their crops, so no hover state shows.

## Settled on 2026-09-20

- **The film is linked.** `LOCANT_FILM_URL` is https://www.youtube.com/watch?v=gIjtllxV-gI, the three-minute Show & Tell film ("Locant: Point, don't describe.", uploaded Sep 15), and the "Watch the film" button now shows in Measured beside the site and GitHub. The video is unlisted, so the link is the only way to it. It stays a link and is never embedded, because its voice-over claims sole authorship where this page does not.
- **Claude Code stays the terminal.** Malik settled on the take that exists rather than shooting the Claude app, so the tile's fourth reading is Cursor, Claude Code (terminal), Codex app, Antigravity, and Final Design's clip is the Codex app then Antigravity.
