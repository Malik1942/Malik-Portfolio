...
function SectionNavigationStage() {
  ...
  return (
    <div className="grid gap-5 md:grid-cols-[180px_minmax(0,1fr)]">
      <div className="flex gap-2 overflow-x-auto border-b border-hairline pb-3 md:block md:space-y-1 md:border-b-0 md:border-r md:pb-0 md:pr-5">
        {sections.map((section) => (
          <button
            ...
            className={`min-h-[40px] shrink-0 rounded-sm px-3 text-left text-caption transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus md:block md:w-full ${active === section ? "bg-foreground/[0.09] text-foreground" : "text-foreground-tertiary hover:text-foreground-secondary"}`}
          >
            {section}
          </button>
        ))}
      </div>
      <div className="min-h-[150px] rounded-sm border border-hairline bg-secondary/[0.08] p-5 sm:p-6">
...
function TransitionStage() {
  ...
  return (
    <div data-testid="transition-stage" data-phase={phase} data-reduced-motion={reduce ? "true" : "false"} className="space-y-5">
      <div className="min-h-[144px] overflow-hidden rounded-sm border border-hairline bg-secondary/[0.08] p-5 sm:p-6">
        <motion.div
          initial={false}
      ...
      <button type="button" onClick={replay} className="min-h-11 rounded-sm border border-hairline px-4 text-sm text-foreground-secondary hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus">
        Replay transition
      </button>
    </div>
  );
...
function ResponsiveStage() {
  ...
  return (
    <div data-testid="responsive-stage" data-layout={layout} className="space-y-5">
      <div className="flex gap-2"><button type="button" onClick={() => setLayout("wide")} className={`min-h-[40px] rounded-sm border px-3 text-caption focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus ${layout === "wide" ? "border-foreground/60 text-foreground" : "border-hairline text-foreground-tertiary"}`}>Wide layout</button><button type="button" onClick={() => setLayout("compact")} className={`min-h-[40px] rounded-sm border px-3 text-caption focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus ${layout === "compact" ? "border-foreground/60 text-foreground" : "border-hairline text-foreground-tertiary"}`}>Compact layout</button></div>
      <div className={layout === "wide" ? "grid gap-3 sm:grid-cols-2" : "space-y-3"}>{["Evidence", "Reflection"].map((title) => <article key={title} className="min-h-[110px] rounded-sm border border-hairline bg-secondary/[0.08] p-4"><p className="text-label uppercase tracking-eyebrow text-foreground-tertiary font-mono">{title}</p><p className="mt-4 text-sm text-foreground-secondary">The same content keeps its order while the relationship changes.</p></article>)}</div>
    </div>
  );
}
...
function AccessibilityStage() {
  ...
  return (
    <div className="space-y-5"><div className="flex flex-wrap gap-3">{controls.map((label) => <button key={label} type="button" onFocus={() => setFocused(label)} className="min-h-11 rounded-sm border border-hairline px-4 text-sm text-foreground-secondary hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus">{label}</button>)}</div><p role="status" className="text-sm text-foreground-secondary">{focused === "None" ? "Tab through the controls to inspect the visible focus path." : `${focused} is focused`}</p></div>
  );
}
