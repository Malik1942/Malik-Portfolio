    ...
    <section data-testid="portfolio-preview" aria-labelledby="portfolio-preview-heading">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-foreground-tertiary">Same-origin production routes with your local CSS-variable patch.</p>
        </div>
        <div role="group" aria-label="Preview viewport" className="flex gap-2">
          {VIEWPORTS.map((viewport) => <button key={viewport} type="button" aria-pressed={width === viewport} onClick={() => setWidth(viewport)} className="min-h-11 rounded-lg border border-border px-3 font-mono text-caption text-foreground-secondary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus">{viewport}px</button>)}
        </div>
      </div>
      <div className="mt-5 overflow-x-auto rounded-lg border border-hairline bg-card/20 p-2 sm:p-4">
      ...
      <a href="/?design-preview=local" className="mt-4 inline-flex min-h-11 items-center text-sm text-foreground-secondary underline underline-offset-4 hover:text-foreground">Open full-site preview</a>
    </section>
  );
}
